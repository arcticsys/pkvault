//import { ApiVaultGetResponse, ApiVaultPostResponse, ApiVaultPostRequest, ErrorResponse } from '@/app/lib';
...
export async function GET() {
    try {
        ...
        //return NextResponse.json({ data } as ApiVaultGetResponse, { status: 200 });
        return NextResponse.json({ data }, { status: 200 });
    } catch (error) {
        console.error('[GET] Failure in /api/vault:', error);
        //return NextResponse.json({ error: '[GET] Failure in /api/vault' } as ErrorResponse, { status: 500 });
...
export async function POST(request: Request) {
    try {
        ...
        if (!body || typeof body !== 'object') {
            //return NextResponse.json({ error: '[POST] No body or invalid type in /api/vault' } as ErrorResponse, { status: 400 });
        ...
        if (!result) {
            //return NextResponse.json({ error: '[POST] Failed to create entry in /api/vault' } as ErrorResponse, { status: 500 });
            return NextResponse.json({ error: '[POST] Failed to create entry in /api/vault' }, { status: 500 });
        }
        //return NextResponse.json({ success: true } as ApiVaultPostResponse, { status: 200 });
        return NextResponse.json({ success: true }, { status: 200 });
    } catch (error) {
        console.error('[POST] Failure in /api/vault:', error);
        //return NextResponse.json({ error: '[POST] Failure in /api/vault' } as ErrorResponse, { status: 500 });
